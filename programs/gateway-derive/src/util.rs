...
use crate::{AccountInfo, ErrorCode, Fee, Pubkey, id};
...
type ParsedGatewayTokenAccountWithFee<'a, 'b> = (GatewayToken, u64, Option<Fee>, &'b AccountInfo<'a>);
...
pub fn check_has_matching_gateway_token(gateway_tokens: &Vec<ParsedGatewayTokenAccountWithFee>, gatekeeper_network: &Pubkey, expected_owner: &Pubkey) -> Result<(), Error> {
  let found_gateway_token = gateway_tokens.iter().find(|(gateway_token, _, _, _)| matches_gatekeeper_network(gateway_token, gatekeeper_network));
...
fn parse_fee_account<'a, 'b>(
  ...
  let expected_fee_account = derive_fee_address(gatekeeper, gatekeeper_network, fee_bump)
    .or(Err(error!(ErrorCode::InvalidFeeAccount)))?;
...
fn parse_accounts<'a, 'b>(accounts: &'b [AccountInfo<'a>], fee_bumps: &[u8]) -> Result<Vec<ParsedGatewayTokenAccountWithFee<'a, 'b>>, Error> {
  let gateway_token_count = accounts.len() / 3;

  msg!("Parsing all {} accounts ({} gateway tokens expected)", accounts.len(), gateway_token_count);

  accounts[..gateway_token_count].iter().enumerate().map(|(i, account)| {
    let gateway_token = Gateway::parse_gateway_token(account).or(Err(error!(ErrorCode::InvalidComponentPass)))?;
    ...
    require!(*gatekeeper.key == gateway_token.issuing_gatekeeper, ErrorCode::GatekeeperMismatch);

    Ok((gateway_token, balance, fee.map(|f| f.into_inner()), gatekeeper))
...
pub fn fee_per_gatekeeper<'a, 'b>(gateway_tokens_with_fee: Vec<ParsedGatewayTokenAccountWithFee<'a, 'b>>) -> HashMap<Pubkey, (&'b AccountInfo<'a>,u64)> {
...
pub fn get_validated_component_passes<'a, 'b, 'c>(
  ...
  let parsed_gateway_tokens_with_fee = parse_accounts(
    accounts,
    fee_bumps
  )?;
...
pub fn validate_empty(account: &AccountInfo, system_program: &Program<System>) -> Result<(), Error> {
  ...
  require!(account.owner == system_program.key, ErrorCode::NonEmptyAccount);
  Ok(())
}

pub fn derive_fee_address(gatekeeper: &Pubkey, gatekeeper_network: &Pubkey, bump: u8) -> Result<Pubkey, Error> {
...
pub fn pay_gatekeepers<'a, 'b>(
  ...
  let fee_map: HashMap<Pubkey, (&'b AccountInfo<'a>,u64)> = fee_per_gatekeeper(parsed_gateway_tokens);
  let mut total_fee = 0;

  fee_map.iter().map(|(_, (gatekeeper, fee))| {
    let account_infos = &[
      payer.to_account_info(),
      gatekeeper.clone().to_account_info(),
      system_program.clone(),
    ];
    msg!("Paying {} lamports from {} to {}", fee, payer.key, gatekeeper.key);
    invoke(
